...
(function(root, factory) {
    if (typeof define === "function" && define.amd) {
        define(['require', 'util', 'underscore', 'sfdb'], factory);
    ...
    var isPredBad = function (predicate, type, category) {
        ...
        if (type === "blueprint") {
            ...
            if (allowedDirTypes.indexOf(pred.directionType) < 0) {
                return "directionType was '" + pred.directionType + "' but it should have been one of " + allowedDirTypes;
            }
            ...
            if (pred.operator !== undefined && okayOps.indexOf(pred.operator) < 0) {
                return "unrecognized operator: '" + pred.operator + "'. In " + type + " predicates, the only valid operators are " + okayOps;
            }
